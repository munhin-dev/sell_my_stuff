...
export default function Product({ onCartUpdate, cart }) {
  ...
  const handleCart = () => {
    if (
      cart.map(({ item }) => item.id).includes(Number(id)) &&
      cart.length !== 0
    ) {
  ...
  const checkDisable = (id) =>
    cart[index(id)]?.quantity >= item.quantity ||
    cart[index(id)]?.quantity + quantity > item.quantity;
  ...
  return (
    ...
    >
      <img
        ...
        alt=""
        style={{ maxHeight: "25rem", width: "100%", objectFit: "scale-down" }}
      />
